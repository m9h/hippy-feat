#!/usr/bin/env python3
"""fMRIPrep BOLD + 4mm FWHM Gaussian spatial smoothing at GLM time.
...
as the field-typical range; the 8mm endpoint hurts every tier × subset
(`SMOOTHING_8MM.md`). This cell tests the 4mm bottom of the bracket —
gentler kernel that still crosses voxel boundaries — to confirm the
smoothing failure isn't kernel-size-specific.

Output cells (suffix `_sm4mm` to disambiguate):
  RT_paper_Fast_pst5_fmriprep_sm4mm_inclz_ses-03
  RT_paper_Slow_pst20_fmriprep_sm4mm_inclz_ses-03
  RT_paper_EoR_fmriprep_sm4mm_inclz_ses-03
"""
...
CONFIGS = [
    ("RT_paper_Fast_pst5_fmriprep_sm4mm_inclz",  5),
    ("RT_paper_Slow_pst20_fmriprep_sm4mm_inclz", 20),
    ("RT_paper_EoR_fmriprep_sm4mm_inclz",        None),  # full-run
]
...
for cell, pst in CONFIGS:
    ...
    if out.exists():
        ...
        continue
    print(f"\n=== {cell} (fmriprep + 4mm + AR(1) LSS, pst={pst}) ===", flush=True)
    t0 = time.time()
