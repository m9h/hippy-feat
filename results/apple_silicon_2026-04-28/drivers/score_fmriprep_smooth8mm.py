#!/usr/bin/env python3
"""Score 8mm-smoothed fmriprep cells (Fast/Slow/EoR) at subset0/1/2 vs the
...
CELLS = [
    ("Fast", "RT_paper_Fast_pst5_fmriprep_sm8mm_inclz",  "RT_paper_Fast_pst5_fmriprep_inclz"),
    ("Slow", "RT_paper_Slow_pst20_fmriprep_sm8mm_inclz", "RT_paper_Slow_pst20_fmriprep_inclz"),
    ("EoR",  "RT_paper_EoR_fmriprep_sm8mm_inclz",         "RT_paper_EoR_fmriprep_inclz"),
]
...
for label, smooth_prefix, base_prefix in CELLS:
    ...
    for n in (0, 1, 2):
        ...
        if k in sm and k in bs:
            ...
            print(f"  {slabel:13s}: 8mm I={sm[k]['image']*100:5.1f}%  vs unsm I={bs[k]['image']*100:5.1f}%  ΔI={di:+5.1f}pp  |  8mm B={sm[k]['brain']*100:5.1f}%  vs unsm B={bs[k]['brain']*100:5.1f}%  ΔB={db:+5.1f}pp", flush=True)
    results[label] = row

(LOCAL / "task_2_1_betas/smoothing_8mm_subsets_fold0.json").write_text(json.dumps(results, indent=2))
print(f"\nsaved smoothing_8mm_subsets_fold0.json", flush=True)

print("\n========== SUMMARY: 8mm fmriprep smoothing vs unsmoothed ==========")
print(f"{'Tier':6s} {'Subset':12s} {'8mm':>8s} {'unsm':>8s} {'ΔI':>6s}    {'8mmB':>6s} {'unsmB':>7s} {'ΔB':>6s}")
