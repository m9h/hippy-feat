#!/usr/bin/env python3
"""fMRIPrep BOLD + plain AR(1) LSS at Fast (pst=5) and Slow (pst=20) latencies.

Tests whether fmriprep BOLD source closes any single-rep gap at shorter
latencies. Compare against rtmotion equivalents:

  rtmotion Fast (RT_paper_Fast_pst5):       single-rep 36, complete-set 44
  rtmotion Slow (RT_paper_Slow_pst20):      single-rep 44, complete-set 72
"""
...
for cell, pst in [("RT_paper_Fast_pst5_fmriprep_inclz", 5),
                  ("RT_paper_Slow_pst20_fmriprep_inclz", 20)]:
    if (R.OUT_DIR / f"{cell}_{SESSION}_betas.npy").exists():
        print(f"=== {cell} already exists — skip ===")
        continue
    print(f"\n=== {cell} (fmriprep + AR(1) LSS, pst={pst}) ===", flush=True)
    t0 = time.time()
