#!/usr/bin/env python3
"""DeepMriPrep aCompCor without erosion — test whether DL segmentation's
tighter boundaries make erosion unnecessary."""
...
P2 = DMP_DIR / "p2sub-005_desc-preproc_T1w.nii.gz"
P3 = DMP_DIR / "p3sub-005_desc-preproc_T1w.nii.gz"
...
print("=== building DeepMriPrep noise pool (NO erosion) ===")
...
print(f"  uneroded CSF∪WM ∩ brain: {csfwm_3d.sum()} voxels", flush=True)
...
ts_per_run = extract_ts(csfwm_3d)
print(f"  ts shape per run: {ts_per_run[0].shape}", flush=True)
...
CELL = "RT_paper_EoR_K7_deepmriprep_HP_inclz"
print(f"\n=== {CELL}  K={K}, HP=on, NO erosion, n_pool={csfwm_3d.sum()} ===", flush=True)
t0 = time.time()
...
config.update({
    ...
    "GLMdenoise_K": K,
    "GLMdenoise_pool": "DeepMriPrep p2 (WM) ∪ p3 (CSF) > 0.5, NO erosion",
    "noise_pool_voxels": int(csfwm_3d.sum()),
