...
| **Spatial smoothing (Gaussian 8mm FWHM)** | `RT_paper_{Fast_pst5,Slow_pst20,EoR}_fmriprep_sm8mm_inclz` | 3 cells | **catastrophic at every tier × subset (-14 to -24pp Image, -8 to -36pp Brain).** Reliability-mask voxels lose pattern fidelity to unreliable neighbors. See `SMOOTHING_8MM.md`. Confirms the paper's `smoothing_fwhm=None` design decision |
...
1. ~~**Spatial smoothing (Gaussian FWHM)**~~ — **TESTED 2026-05-08** at 8mm on all three tiers, see `SMOOTHING_8MM.md`. Catastrophic: -14 to -36pp at every tier × subset. The `smoothing_fwhm=None` paper default is correct on a reliability mask
